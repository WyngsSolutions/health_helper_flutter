...
class _MapSearchState extends State<MapSearch> {
  ...
  void initState() {
    ...
    //   ImageConfiguration(size: Size(40, 40)), 'assets/logo_hospital.png')
    //   .then((onValue) {
    //     hospitalIcon = onValue;
    // });
  }

  Future<void> _onMapCreated(GoogleMapController cntlr)
  async {
    controller = cntlr;
    ...
    for(int i=0; i < hospitals.length; i++)
    {
      ...
      Marker marker = Marker(
        markerId: MarkerId("hospitals$i"),
        position: LatLng(hospitals[i]['latitude'], hospitals[i]['longitude']),
        infoWindow: InfoWindow(
          title: "${hospitals[i]['name']}",
          snippet: '${hospitals[i]['address']}'
        ),
        draggable: false,
  ...
  void getNearbyHospitals()async{
    ...
    dynamic result = await AppController().getNearbyHospitals(userLatitude, userLongitude);
    EasyLoading.dismiss();
